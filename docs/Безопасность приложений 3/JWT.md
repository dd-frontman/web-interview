...
- содержит *claims* — утверждения (например: кто пользователь, его роль, когда токен истёк и др.) 
...
2. **Payload** — полезная нагрузка, JSON-объект, где перечислены *claims*. В payload часто встречаются стандартные (зарегистрированные) claims:
   ...
   Кроме стандартных, можно добавлять **кастомные claims** — например, роль пользователя, права доступа и др. :contentReference[oaicite:12]{index=12}
...
3. **Signature** (подпись) — обеспечивает, что header и payload не были изменены. Обычно создаётся так:
   ...
   Алгоритм зависит от `alg` в header. :contentReference[oaicite:13]{index=13}
...
- ИETF, RFC 7519 — официальный стандарт JWT. :contentReference[oaicite:16]{index=16}  
- jwt.io — вводная информация, примеры, инструменты. :contentReference[oaicite:17]{index=17}  
- Auth0, Descope и прочие ресурсы, объясняющие типичные claims и использование. :contentReference[oaicite:18]{index=18}  
- Wikipedia — общее понятие, историческая справка. :contentReference[oaicite:19]{index=19}  
