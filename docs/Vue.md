...
> - [[React на примере Vue|React на примере Vue]]
> - [[Таблица сравнения React vs Vue|Таблица сравнения React vs Vue]]
> - [[defineExpose()|defineExpose()]]
> - [[provide и inject|provide и inject]]
  ...
  getters: {
    filteredTodos: (state) => {
      ...
      return state.todos.filter(
        (todo) => todo.completed === (state.filter === "completed")
      );
    },
  },
...
```html
<div v-for="item in items" :key="item.id" v-memo="[item.id, item.completed]">
  {{ item.text }}
</div>
```
...
> - [[Рендер-функции|Рендер-функции]]
> - [[Программирование/Nuxt2 vs Nuxt3|Nuxt2 vs Nuxt3]]
