...
**Vue → SFC `scoped`**
```vue
<style scoped>.btn{ padding:8px }</style>
```
...
```jsx
const elRef = useRef();
useEffect(() => { externalLib.init(elRef.current); }, []);
<div ref={elRef} />
```
**Vue → `onMounted` + refs**
```vue
...
onMounted(() => { externalLib.init(elRef.value) })
</script>
...
```jsx
<Layout header={<Header />} footer={<Footer />}>Main</Layout>
```
...
try { await fetchData() } catch(e){ error.value = e }
</script>
```
...
```jsx
setA(1); setB(2); // один рендер
```
**Vue → реактивность автоматически группирует обновления**
```js
a.value = 1; b.value = 2; // один рендер
```
...
onMounted(() => { socket = new WebSocket(url) })
