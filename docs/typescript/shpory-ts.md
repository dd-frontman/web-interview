...
- never (Такой тип получают выражения, которые не могут завершиться нормально: функции, выбрасывающие исключение, бесконечные циклы, исчерпывающие проверки switch/if, где последняя ветка «невозможна». Пример: function fail(msg: string): never { throw new Error(msg); }. Типизация never помогает компилятору гарантировать, что вы обработали все возможные варианты объединения; если появляется ветка, которая по идее «никогда не случится», её тип — never, и TS сможет подсветить пропущенные случаи)
...
В TypeScript типы (`type`) и интерфейсы (`interface`) имеют много общего, но есть и ключевые различия. Вот основные из них:
...
Это **готовые дженерик-типы**, которые позволяют преобразовывать или извлекать типы на уровне компиляции — например, делать свойства опциональными или убирать `null` из объединения.
