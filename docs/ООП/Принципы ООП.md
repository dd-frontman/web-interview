...
### Классы JS
```ts
class Animal { speak() { console.log("звук"); } }
class Dog extends Animal { speak() { console.log("Гав"); } }
class Cat extends Animal { speak() { console.log("Мяу"); } }
...
### React
```tsx
function Dog() { return <p>Гав</p>; }
function Cat() { return <p>Мяу</p>; }
